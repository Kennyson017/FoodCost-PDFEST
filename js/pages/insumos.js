...
export const renderInsumos = (container, params = []) => {
    // Roteamento interno (ex: #insumos/novo, #insumos/123/editar)
    if (params.length > 0) {
        if (params[0] === 'novo') {
...
const renderList = (container) => {
    ...
    container.innerHTML = `
        <div class="header-action" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
            <div>
                <h1>Insumos</h1>
                <p style="color: var(--text-muted);">${insumos.length} itens cadastrados</p>
            </div>
                ...
                <tbody>
                    ${insumos.map(i => `
                        <tr>
                            ...
                            <td><span class="badge badge-neutral">${escapeHTML(i.categoria)}</span></td>
                            <td>${i.quantidadeCompra} ${escapeHTML(i.unidadeCompra)}</td>
                            <td>${Calc.formatCurrency(i.precoCompra)}</td>
                            <td><strong style="color: var(--primary);">${Calc.formatCurrency(i.custoUnitarioBase)} / ${escapeHTML(i.unidadeBase)}</strong></td>
...
const renderForm = (container) => {
    ...
    [inputPreco, inputQtd, inputUnidade].forEach(el => el.addEventListener('input', updatePreview));
